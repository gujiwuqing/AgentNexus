...
describe("streamAgentReply", () => {
  it("builds an OpenAI-compatible client from the provider config and calls streamText", () => {
    const provider = { baseUrl: "https://api.example/v1", model: "gpt-test", apiKey: "key-123" };
    const messages = [{ role: "user" as const, content: "hi" }];
    const onFinish = vi.fn();

    streamAgentReply(provider, messages, { temperature: 0.5, maxTokens: 500, topP: 0.9 }, onFinish);
  ...
  it("wires onFinish to forward text and usage", async () => {
    ...
    streamAgentReply(provider, [], { temperature: 0.7, maxTokens: 1024, topP: 1 }, onFinish);
