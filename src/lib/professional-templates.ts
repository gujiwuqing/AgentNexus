...
export const professionalAgentTemplates = [
  {
    ...
    model: null,
    suggestedPrompts: [
      '帮我调研这个技术选型，对比主流方案的优劣势：{{填入选型主题}}',
      '验证这个说法是否有可靠依据：{{填入待核实的说法}}',
      '帮我梳理这个领域的研究现状和关键争议：{{填入领域或问题}}',
      '对这份报告的结论做一次证据审查：\n{{粘贴报告内容}}',
    ],
  },
  {
  ...
  },
  {
    ...
    model: null,
    suggestedPrompts: [
      '为这个功能上线制定一份 4 周运营计划：{{填入功能名称}}',
      '帮我设计这次活动的核心指标和监测口径：{{填入活动名称}}',
      '针对这个指标下降给出可执行的运营对策：{{填入指标与现状}}',
      '帮我做一次活动复盘的结构化分析：\n{{粘贴活动数据与过程}}',
    ],
  },
  {
    ...
    model: null,
    suggestedPrompts: [
      '帮我设计这个功能的技术方案和分期计划：{{填入功能描述}}',
      '评估这两种架构选型的关键取舍：{{填入方案 A 与方案 B}}',
      '帮我梳理这次发布的回滚预案和风险点：{{填入发布内容}}',
      '把这个业务目标翻译成模块和接口设计：{{填入业务目标}}',
    ],
  },
  {
    key: 'code-reviewer',
  ...
  },
  {
    ...
    model: null,
    suggestedPrompts: [
      '为这个功能设计基于风险的测试计划：{{填入功能描述}}',
      '帮我找出这个流程的边界用例和异常场景：{{填入流程描述}}',
      '梳理这次发布的准入检查项：{{填入发布范围}}',
      '评估这份需求的可测性风险：\n{{粘贴需求}}',
    ],
  },
